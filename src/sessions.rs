...
pub(crate) struct OutboundSequencer {
    ...
    close_meta: Arc<Mutex<Option<EventMeta>>>,
    task: JoinHandle<()>,
}
...
impl OutboundSequencer {
    pub(crate) fn new(
        ...
        let close_meta = Arc::new(Mutex::new(None));
        let task = task_runtime.spawn(run_sequencer(SequencerResources {
            ...
            opened,
            stop: stop.clone(),
            close_meta: Arc::clone(&close_meta),
        }));
        Self {
            completion_input,
            stop,
    ...
    pub(crate) async fn close(self, meta: EventMeta) -> Result<(), AgentError> {
        let Self {
            completion_input,
            stop,
            close_meta,
...
async fn run_sequencer(resources: SequencerResources) {
    ...
    #[cfg(test)]
    if let Some(gate) = take_sequencer_gate(&opened.workspace) {
...
impl LoadedSession {
    pub(crate) async fn shutdown(self, meta: EventMeta) -> Result<(), AgentError> {
        ...
        let sequencer_error = sequencer.close(meta).await.err();
